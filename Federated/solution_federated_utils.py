...
from utilities.feature_generation_pipeline import *
import utilities.utils as utils
import utilities.utils_he as homomorphic_encryption

from pandemic_model import train_model,test_model

epochs = 5
lr=0.001
privacy_budget = 9 #define between 1 to 10 (1 => very strict budget)
mu = 0.01
max_grad_norm = 10
batch_size = 128
threshold = 0.5
...
#         pk, sk = KeyGen()
        ...
        # Create Model
        self.model = utils.LogisticRegression()
...
#         decoded_parameters = homomorphic_encryption.decrypt_weights_param(self.sk, parameters, num_clients)
        ...
        self.model = utils.set_model_params(self.model, decoded_parameters)
        
        # Ignore convergence failure due to low local epochs
        with warnings.catch_warnings():
            warnings.simplefilter("ignore") 
            self.model = utils.fit(self.model, decoded_parameters, mu, self.X_train, self.y_train, 
        privacy_budget, max_grad_norm, lr,batch_size,epochs)
            
        #encryption
        local_model_parameters = utils.get_model_parameters(self.model)
...
#             pickle.dump(decoded_parameters, f)
        ...
        predictions, loss, accuracy, avg_precision = utils.test_model(self.model, self.X_test, self.y_test,threshold)
        print(f"Loss - {loss}, Accuracy - {accuracy}, Average Precision - {avg_precision}")
       
        return loss, len(self.X_test), {"average_precision": avg_precision}
...
def train_client_factory(
    ...
    final_data_train = generate_modelling_data(train_run_Parameters, dict_of_loc, client_dir, 
                                               db_name = 'my-db.duckdb',tables_loaded = False, add_pats = None)
   
    X = final_data_train.drop(['pid'],axis = 1).fillna(0)
...
#         pk, sk = KeyGen()
    ...
    def fit(self, parameters, config):  # type: ignore
        ...
        return utils.get_model_parameters(self.model), len(self.X_test), {}

    def evaluate(self, parameters, config):  # type: ignore
        print("Test Client Evaluate Fn called")
   
        self.model = utils.set_model_params(self.model, parameters)
        predictions, loss, accuracy, avg_precision = utils.test_model(self.model, self.X_test, self.y_test, self.threshold)
        print(f"Loss - {loss}, Accuracy - {accuracy}, Average Precision - {avg_precision}")
...
def test_client_factory(
    ...
    X = final_data_test.drop(['pid'],axis = 1).fillna(0)
