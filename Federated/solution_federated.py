...
from .feature_generation_pipeline_withoutduckdb import generate_modelling_data
from .utils import *
...
epochs = 5
lr=0.001
privacy_budget = 3 #define between 1 to 10 (1 => very strict budget)
mu = 0.01
max_grad_norm = 10
batch_size = 128
threshold = 0.5
developer_testing = False
...
#         pk, sk = KeyGen()
        ...
        # Create Model
        self.model = LogisticRegression(param_count=X_train.shape[1])
...
def train_client_factory(
    ...
    final_prepared_data_path = client_dir.joinpath('final_prepared_data_train.csv')
    if os.path.exists(final_prepared_data_path):
        final_data_train = pd.read_csv(final_prepared_data_path)
    else:
        final_data_train = generate_modelling_data(dict_of_loc, end_day = 49,fixed_ending = 49,window_lenght = 2,is_training = True)
        final_data_train.to_csv(final_prepared_data_path, index=False)
...
#         params_server_dir_path = pathlib.Path(self.server_dir, "model_param.npz")
#         if agg_params is not None:
#             # Convert `Parameters` to `List[np.ndarray]`
#             aggregated_ndarrays = fl.common.parameters_to_ndarrays(agg_params)

#             # Save aggregated_ndarrays
#             print(f"Saving round {server_round} aggregated_ndarrays...")
#             np.savez(params_server_dir_path, *aggregated_ndarrays)
...
class TestClient(fl.client.NumPyClient):
    ...
    def __init__(self, client_dir, preds_format_path, preds_dest_path, data, X_test, y_test, true_label=None):
...
def test_client_factory(
    ...
    final_prepared_data_path = client_dir.joinpath('final_prepared_data_test.csv')
    if os.path.exists(final_prepared_data_path):
        final_data_test = pd.read_csv(final_prepared_data_path)
    else:
        final_data_test = generate_modelling_data(dict_of_loc, end_day = 50,fixed_ending = 49,window_lenght = 2,is_training = False)
        final_data_test.to_csv(final_prepared_data_path, index=False)

    #final_data_test = generate_modelling_data(dict_of_loc, end_day = 50,fixed_ending = 49,window_lenght = 2,is_training = False)
    #final_data_test = hacky_feature_banao(dict_of_loc)
    true_label = None
    if developer_testing:
        true_label = prepare_developer_true_label(dict_of_loc)
    ...
    return TestClient(client_dir, preds_format_path, preds_dest_path, final_data_test, X_std, y, true_label)
