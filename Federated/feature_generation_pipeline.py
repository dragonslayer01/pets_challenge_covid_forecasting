...
def run_query(cursor, statement):
    cursor.execute(statement)
    cursor.close()
    
def get_data(cursor, statement):
    df = cursor.execute(statement).fetch_df()
    cursor.close()
    return df
...
def covid_pats_timeline(cursor, train_Parameters):
    '''
...
def asym_covid_pats_timeline(cursor, train_Parameters):
...
def generate_covid_pats_regress(cursor):
    statement = '''
    ...
    recovery_data = get_data(cursor, statement)
    return recovery_data

# recovery_data = generate_covid_pats_regress()

def generate_noncovid_pats_regress(cursor):
    statement = '''
    select a.pid,sex,age,person_number,
    max(person_number) over(partition by hid) as total_family_members,
    last_day
    ...
    asym_recovery_data = get_data(cursor, statement)
    return asym_recovery_data

# asym_recovery_data = generate_noncovid_pats_regress()


def predict_revocery_time_noncovid(cursor, asym_recovery_data,clf,scaler):
    
    if clf == 2:
        asym_recovery_data['first_day'] = asym_recovery_data.last_day - clf
...
def merge_sympt_asympt_pats(cursor, train_Parameters,add_pats = None):
...
def contact_network_covid(cursor):
    '''
...
def window_based_feature_covid(cursor, run_Parameters):
...
def window_based_feature_noncovid(cursor, run_Parameters):
...
def window_based_feature_noncovid_inference(cursor, run_Parameters):
...
def calculate_day_level_feature(cursor, run_Parameters,day = 1,day_str = 'lag_day',
...
def interaction_based_features(cursor, run_Parameters):
    contact_network_covid(cursor)
    cursor = create_cursor()
    window_based_feature_covid(cursor, run_Parameters)
    cursor = create_cursor()
    window_based_feature_noncovid(cursor, run_Parameters)
    
    if run_Parameters.is_training == False:
        pass
    else:
        cursor = create_cursor()
        window_based_feature_noncovid_inference(cursor, run_Parameters)
    ...
    for i in range(run_Parameters.window_lenght):
        cursor = create_cursor()
        feature_1 = calculate_day_level_feature(cursor, run_Parameters,day = i,table = 'pats_interaction_feature',
        ...
        if run_Parameters.is_training == True:
            cursor = create_cursor()
            feature_2 = calculate_day_level_feature(cursor, run_Parameters,day = i,table = 'noncovid_pats_inference_interaction_feature',
                                   feature = 'total_interactions',
                                   final_feature_name = 'infected_interactions',
...
def create_location_based_feature(cursor, run_Parameters):
...
def location_based_features(cursor, run_Parameters):
...
def create_family_feature(cursor, run_Parameters):
...
def family_based_features(cursor, run_Parameters):
    ...
    run_query(cursor, statement)
    cursor = create_cursor()
    create_family_feature(cursor, run_Parameters)
    ...
    for i in range(run_Parameters.window_lenght):
        cursor = create_cursor()
        feature_1 = calculate_day_level_feature(cursor, run_Parameters,day = i,table = 'family_feature',
                                   feature = 'family_infected',
                                   final_feature_name = 'family_infected',
                                    master_table = 'family_feature_lag'
                                   )
        cursor = create_cursor()
        feature_2 = calculate_day_level_feature(cursor, run_Parameters,day = i,table = 'noncovid_recovered_pats_family_inference',
                                   feature = 'family_infected',
                                   final_feature_name = 'family_infected',
...
def create_train_test_pats(cursor, run_Parameters):
...
def generate_master_data(cursor, feature_list,is_inference = False):
...
def final_master_data(cursor, run_Parameters,train_feat_list,infer_feat_list):
    
    is_training = run_Parameters.is_training
    
    statement = generate_master_data(cursor, train_feat_list,is_inference = False)

    run_query(cursor, statement)
    cursor = create_cursor()
    statement = generate_master_data(cursor, infer_feat_list,is_inference = True)
    cursor = create_cursor()
    run_query(cursor, statement)
    ...
    if is_training:
        cursor = create_cursor()
        final_data = get_data(cursor, 'select * from patient_level_features_master_training')
        # cursor.close()
        return final_data
    else:
        cursor = create_cursor()
        final_data_1 = get_data(cursor, 'select * from patient_level_features_master_testing')
        cursor = create_cursor()
        final_data_2 = get_data(cursor, 'select * from patient_level_features_master_inference')
        # cursor.close()
        return final_data_1,final_data_2
    
def create_table_duck_db(cursor, table, file_loc):
    statement = f'''
        drop table if exists {table};
        create table {table} as
        select * from read_csv_auto('{file_loc[table]}');
    '''
...
def create_cursor():
    cursor = duckdb.connect(database=str(ip_db_path),read_only=False)
    return cursor

def generate_modelling_data(run_Parameters,file_loc,client_dir,db_name = 'my-db.duckdb',tables_loaded = True,add_pats = None):
    
    
    # Data Loading
    client_dir = client_dir.resolve()
    print("CLIENT DIRR ::", client_dir)
    ...
    print("PATH BEING PASSED ::::", db_path)
    cursor = duckdb.connect(database=str(db_path),read_only=True)
    ...
    if tables_loaded:      
        ...
        for i in file_loc.keys():
            create_table_duck_db(cursor, i, file_loc)
    
    # Feature Generation Pipeline
    covid_pats_timeline(cursor, run_Parameters)
    cursor = create_cursor()
    asym_covid_pats_timeline(cursor, run_Parameters)
    
    # Training Linear Regression for recovery time
    recovery_data = generate_covid_pats_regress(cursor)
    cursor = create_cursor()
    clf,scaler = run_logistic_recovery(cursor, recovery_data)
    cursor = create_cursor()
    asym_recovery_data = generate_noncovid_pats_regress(cursor)
    cursor = create_cursor()
    predict_revocery_time_noncovid(cursor, asym_recovery_data,clf,scaler)
    
    # Creating Features (Interaction + Location + Family)
    cursor = create_cursor()
    merge_sympt_asympt_pats(cursor, run_Parameters,add_pats)
    cursor = create_cursor()
    interaction_feats = interaction_based_features(cursor, run_Parameters)
    cursor = create_cursor()
    loca_feats = location_based_features(cursor, run_Parameters)
    cursor = create_cursor()
    family_feats = family_based_features(cursor, run_Parameters)
    cursor = create_cursor()
    create_train_test_pats(cursor, run_Parameters)
    cursor = create_cursor()
...
#     cursor.close()
    ...
    return final_master_data(cursor, run_Parameters,train_feat_list,infer_feat_list)